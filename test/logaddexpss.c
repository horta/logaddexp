...
int main(void)
{
    if (test_logaddexpss_values()) return 1;

    if (test_logaddexpss_dbl_max()) return 1;

    if (test_logaddexpss_range_pos()) return 1;

    if (test_logaddexpss_range_neg()) return 1;

    if (test_logaddexpss_hand_made()) return 1;
...
int test_logaddexpss_values(void)
{
    ...
    for (size_t i = 0; i < 5; ++i)
    {
        ...
        if (fabs(expected_val - v) > EPSILON) return 1;
...
int test_logaddexpss_dbl_max(void)
{
    ...
    if (fabs(-DBL_MAX - logaddexpss(1, 1, 1, -1, &sign)) > EPSILON) return 1;
