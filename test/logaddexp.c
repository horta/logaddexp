...
int main()
{
    ...
    if (test_logaddexp_range())
        return 1;

    if (test_logaddexp_inf())
        return 1;

    if (test_logaddexp_nan())
        return 1;
...
int test_logaddexp_inf(void)
{
    double x[] = {INFINITY, -INFINITY, INFINITY, -INFINITY, INFINITY, 1, -INFINITY, 1};
    double y[] = {INFINITY, INFINITY, -INFINITY, -INFINITY, 1, INFINITY, 1, -INFINITY};
...
int test_logaddexp_nan(void)
{
    if (!isnan(logaddexp(NAN, INFINITY)))
        return 1;

    if (!isnan(logaddexp(INFINITY, NAN)))
        return 1;
